...
const sendCalculatorFeedbackEmail = async (info) => {
	try {
		const { name, feedbackCategory, message, imageBuffer, timestamp, fileName } = info;

		const templatePath = path.join(__dirname, '..', 'templates', 'feedback-email.html');
		let htmlTemplate = await fs.readFile(templatePath, 'utf8');
...
const sendCalculatorHelpfulFeedbackEmail = async (info) => {
	try {
		const { name, feedbackCategory, message, timestamp, calculatorName } = info;

		const templatePath = path.join(__dirname, '..', 'templates', 'feedback-helpful-email.html');
		let htmlTemplate = await fs.readFile(templatePath, 'utf8');
